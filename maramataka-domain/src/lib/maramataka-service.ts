  ...
  async getYear(location: Location, date: Date): Promise<MaramatakaYear> {
    ...
    const [
      ...
      await Promise.all([
        ...
        this.astronomyProvider.getNewMoons(localYear + 1),
        this.astronomyProvider.getFullMoons(localYear - 1),
        this.astronomyProvider.getFullMoons(localYear),
        this.astronomyProvider.getFullMoons(localYear + 1),
      ]);
    const newMoons = [
      ...this.asArray(previousYearNewMoons),
    ...
    const months = (
      await Promise.all(
        yearNewMoonAnchors.map((newMoon, index) =>
